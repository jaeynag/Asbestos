import { createClient } from httpscdn.jsdelivr.netnpm@supabasesupabase-js@2+esm;

 고정 값 
const SUPABASE_URL = httpsjvzcynpajbjdbtzbysxm.supabase.co;
...
async function signUp(email, password) {
  ...
  if (exists === true) {
   ...
   보안설정에 따라 이미 가입된 이메일이어도 성공처럼 응답할 수 있습니다.
...
async function uploadAndBindPhoto(sample, role, file) {
  ...
  try {
    ...
    const { error upErr } = await supabase.storage
      .from(BUCKET)
      .upload(path, file, { upsert true, contentType file.type  imagejpeg });
    ...
    if (sampleIds.length) {
      ...
      if (paths.length) {
        const { error rmErr } = await supabase.storage.from(BUCKET).remove(paths);
         권한RLS로 막힐 수도 있어서, 스토리지 삭제 실패는 경고로만 처리
...
function renderAuth() {
  ...
  const msg = el(div, { style margin-top10px; color#666; font-size13px; });
...
function renderCompanySelect() {
  const list = el(div, { class list, style margin-top12px; });
  root.appendChild(el(section, { class card }, [
    el(div, { class label, text 회사 선택 }),
    el(div, { style margin-top6px; font-weight800;, text 접근 가능한 회사 목록 }),
    list
  ]));
...
function renderModeSelect() {
  root.appendChild(el(section, { class card }, [
    ...
    el(div, { class row, style margin-top10px; }, [
...
function renderJobSelect() {
  ...
  root.appendChild(el(section, { class card }, [
    el(div, { class row space }, [
      el(div, { class col, style flex1; min-width220px; }, [
  ...
  const refresh = async () = {
    ...
    for (const j of filtered) {
      ...
      list.appendChild(el(div, { class item }, [
        ...
        el(div, { class row, style align-itemscenter; gap8px; }, [
          (j.status === new)  el(span, { class badge-new, text NEW })  el(span, { style displaynone; }),
...
function renderJobWork() {
  ...
  root.appendChild(el(section, { class card }, [
    el(div, { class row space }, [
      el(div, { class col }, [
        ...
        el(div, { style font-weight900; font-size16px; margin-top4px;, text state.job.project_name }),
        el(div, { style font-size12px; color#666; margin-top4px;, text state.company.name }),
      ]),
      el(div, { class row }, [
        el(button, {
          class btn small,
  ...
  const tabs = el(div, { class tabs, style margin-top10px; });
  ...
  root.appendChild(el(section, { class card }, [
    ...
    el(div, { class dateControls, style margin-top10px; }, [
  ...
  root.appendChild(el(section, { class card }, [
    el(div, { class row space }, [
      el(div, { class col }, [
        ...
        el(div, { style font-weight800; margin-top4px;, text `${ymdDots(dateISO)} 시료 목록` }),
      ]),
      el(div, { class row }, [
        el(button, {
          class btn primary,
  ...
  if (!samples.length) {
    root.appendChild(el(section, { class card }, [
      ...
      el(div, { style margin-top8px; color#666; font-size13px;, text 해당 날짜에 시료가 없습니다. 시료를 추가해 주세요. })
    ]));
    return;
  }
...
function renderAddSamplePanel(dateISO) {
  ...
  return el(div, { class row, style margin-top12px; }, [
    el(div, { class col, style flex1; min-width220px; }, [
...
function renderMiniSlot(sample, role) {
  ...
  const btns = el(div, { class slotBtns }, [
   ...
   ✅ 농도 라벨(닷+측정사진) + 썸네일을 한 줄로 만들고, 버튼은 아래
